...
// sends it to the owner's Telegram via the single bot.
//
 ...
 * and the raw form. Only telegram envelopes match (swarm-inbox events carry
 ...
 * would be suppressed, losing the owner an answer (FIX 6). So we fold the
 ...
 * suppressed → the owner never gets the answer. So we parse the body and
...
async function main(): Promise<void> {
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    // A reply already reached the owner this turn → never fall back.

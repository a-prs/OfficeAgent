...
export function resolvePolicyPath(env: Readonly<Record<string, string | undefined>>): string {
  ...
  const ws = env.CLAUDE_WORKSPACE_DIR && env.CLAUDE_WORKSPACE_DIR.length > 0
    ...
    : join(env.HOME ?? '', '.claude-lab/agent-c/.claude')
...
export function buildConfirmRequest(args: {
  ...
  // CHAT_ID is the composite session key ("<chatId>_t<threadId>" for a topic
  // session). Pass it so the channel can route the Allow/Deny card to the
  // originating topic instead of the owner DM. The channel re-validates it
  // against the allowlist; an unset/"main" scope falls back to owner DM there.
  ...
  const body = JSON.stringify({
    ...
    ...(chatId !== undefined && chatId !== '' && chatId !== 'main' ? { chat_id: chatId } : {}),
  })
  return {
    url,

...
function buildCommand(opts: PatchOptions): string {
  // Inherit the live session's CHAT_ID so a TOPIC session tags its journal
  // with the composite "<chatId>_t<threadId>" (per-thread attribution),
  // falling back to the configured chat id for the master/non-topic session
  // (which has no CHAT_ID in env). A chat id is numeric (^-?\d+$), so the
  // fallback can't smuggle shell metacharacters into the ${...:-} expansion.
  const fallbackId = opts.chatId.replace(/[^0-9-]/g, '')
  const envParts: string[] = [
    `TELEGRAM_HOOK_CHAT_ID="\${CHAT_ID:-${fallbackId}}"`,
  ]
  if (opts.agentId) {
    envParts.push(
...
function buildGateCommand(opts: PatchOptions): string {
  ...
  // NB: do NOT hardcode CHAT_ID here. The gate hook must inherit the live
  // session's CHAT_ID from env — for a topic session that is the composite
  // "<chatId>_t<threadId>", which lets the channel route the Allow/Deny card
  // back to the originating topic. A hardcoded owner id here would clobber the
  // composite and force every card into the owner DM (regression 2026-06-18).
  // The master/non-topic session has no CHAT_ID → hook omits chat_id → channel
  // falls back to the owner DM, which is the correct behaviour there.
  const envParts: string[] = [

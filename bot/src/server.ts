...
// owner's chat id — when owner DM had `streaming: 'progress'`,
...
// feature is silently unavailable rather than guessing a target.
let modelSwitch: ModelSwitch | null = null
...
if (config.alt_provider.enabled && masterPaneTarget !== undefined) {
  try {
    ...
    modelSwitch = new ModelSwitch({
      ...
      // exactOptionalPropertyTypes) when multichat is disabled; topic
      // switches then degrade to the old bare-claude path (there is no
      // topic pane to switch anyway).
      ...(multichatWorkspaceDir !== undefined && multichatStateDir !== undefined
        ? { multichatWorkspaceDir, multichatStateDir }
        : {}),
      log,
    })
...
// model-switch-state.json lives under the plugin's own state root (not
...
bot.on('callback_query:data', async ctx => {
  ...
  // multi2 patch: terminal keypad (kp:*) under the tmux mirror. Sends a raw
  // keystroke to the mirrored pane so CC-internal prompts (malformed-bash
  // safety, trust dialogs) — which cannot be relayed — can be answered from
  // Telegram. Owner-only; keys are a fixed allowlist (no arbitrary input).
  if (data.startsWith('kp:')) {
    try {
      ...
      const keys = KP[data.slice(3)]
      const allowed = (config.allowed_user_ids ?? []).includes(ctx.from.id)
      // MILESTONE_03: a keypad tap inside a forum TOPIC routes the keystroke to
      // that topic's per-topic session (multichat-<chatId>_t<threadId> on the
      // default tmux socket where the pool spawns), NOT the master pane. The
      // callback message carries the topic via message_thread_id.
      const cbMsg = ctx.callbackQuery.message as
        | { message_thread_id?: number; chat?: { id?: number } }
        | undefined
      const kpThreadId = cbMsg?.message_thread_id
      const kpChatId = cbMsg?.chat?.id
      if (allowed && keys && kpThreadId !== undefined && kpChatId !== undefined) {
        const sessionName = 'multichat-' + String(kpChatId) + '_t' + String(kpThreadId)
        execFileSync('tmux',
      ...
      log.error('keypad callback threw', {
        error: err instanceof Error ? err.message : String(err),
      })
  ...
  if (data.startsWith('model:')) {
    ...
    try {
      ...
      const allowed = (config.allowed_user_ids ?? []).includes(ctx.from.id)
      if (!allowed || !modelSwitch) {
        await ctx.answerCallbackQuery({ text: allowed ? 'модель: не настроено' : 'не авторизовано' })
        return
      }
      const cbMsg3 = ctx.callbackQuery.message as
        | { message_thread_id?: number; chat?: { id?: number } }
        | undefined
      ...
      // guaranteed to run.
      await ctx.answerCallbackQuery({ text: `переключаю на ${label}` })
      const replyChatId = isTopic ? `${String(modelChatId)}_t${String(modelThreadId)}` : String(ctx.from.id)
      await telegramApi.sendMessage(
        replyChatId,
        `<b>переключаю на ${label}</b>${isTopic ? '' : '\n\nконтекст сохранится (--continue). пиши следующее сообщение после того, как отвечу с новой модели.'}`,
        { parse_mode: 'HTML' },
      ).catch(() => {})
      if (isTopic) {
        const topicChatId = `${String(modelChatId)}_t${String(modelThreadId)}`
        const paneTarget = `multichat-${topicChatId}`
        try {
          const sessionId = await resolveTopicSessionId(topicChatId)
          const result = await modelSwitch.switchTo(target, { paneTarget, sessionId })
          if (result.ok) {
            await setModelState(modelSwitchStateFilePath, paneTarget, target, log)
          } else {
            log.warn('model callback: topic switch failed', { paneTarget, error: result.error })
          }
        } catch (err) {
          log.warn('model callback: topic session-id resolution failed', {
            topicChatId,
            error: err instanceof Error ? err.message : String(err),
          })
        }
      } else {
        // Master pane — this call may terminate the current process
        // (tmux respawn-pane -k). Nothing after it is guaranteed to run.
        await modelSwitch.switchTo(target)
      }
    } catch (err) {
...
if (
  ...
  try {
    ...
    // canonical Agent-c layout: `{workspaceDir}/chats`.
    ...
    // staging/Agent-c VPS the canonical `claude` lives at a
    ...
    multichatRouter = new MultichatRouter({
      ...
      telegramApi: {
        ...
        editForumTopic: (chatId, name) =>
          telegramApi.editForumTopic(chatId, name),
        closeForumTopic: (chatId) =>
          telegramApi.closeForumTopic(chatId),
        createForumTopic: (chatId, name) =>
          telegramApi.createForumTopic(chatId, name),
        deleteForumTopic: (chatId) =>
          telegramApi.deleteForumTopic(chatId),
      },
      logger: log,
      // model-switch (2026-07-30): a killed topic pane (idle watchdog or
      // explicit) always cold-boots plain Claude on its next respawn — see
      // model-state.ts's doc comment for why the state file must be reset
      // here rather than left to go stale/lie.
      onSessionKilled: (cid) => {
        void resetModelStateOnKill(modelSwitchStateFilePath, `multichat-${cid}`, log)
      },
    })
    ...
    // Persistent-topic journals: route forum-topic Stop hooks to a
    // per-topic journal under {workspaceDir}/chats/{compositeId}/ instead
    // of the master recent.md. Same dir tree session-start.sh reads back
    // on the next spawn for seamless restore.
    memoryWriter?.setTopicJournalBase(chatsBasePath)
...
const handlerDeps: HandlerDeps = {
  ...
  // (oob.ts) writes to the exact same state file the callback_query handler
  // and webhook routes use — never re-derived independently.
  ...(modelSwitch !== null ? { modelSwitchStateFilePath } : {}),
...
try {
  webhookHandle = await startWebhookServer(config, {
    ...
    // Allowlisted base chat ids — lets the gate route the Allow/Deny card to
    // the originating topic (composite chat_id) when its base chat is trusted,
    // instead of always falling back to the owner DM.
    ...(multichatPolicy ? { permissionAllowedChats: multichatPolicy.allowlist.chats } : {}),
    // model-switch (2026-07-30): dashboard button proxy target + Telegram
    // inline-keyboard target both go through this one capability, backed
    // by the same modelSwitch instance the master's own /model OOB command
    // uses (switchTo's optional TopicSwitchOverride tells them apart).
    ...(modelSwitch !== null && masterPaneTarget !== undefined
      ? { modelSwitchUi: { masterPaneTarget, modelSwitch, stateFilePath: modelSwitchStateFilePath } }
      : {}),
  })
} catch (err) {

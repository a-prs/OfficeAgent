...
export interface TelegramApi {
  ...
  editForumTopic(chatId: string, name: string): Promise<void>
  closeForumTopic(chatId: string): Promise<void>
  createForumTopic(chatId: string, name: string): Promise<number>
  deleteForumTopic(chatId: string): Promise<void>
}
...
export function createTelegramApi(bot: Bot, token: string): TelegramApi {
  // MILESTONE_03 topic-session: decode composite key "<chatId>_t<threadId>"
  // into a real Telegram chat id + message_thread_id, so each outbound lands
  // in its forum topic. Plain chat ids pass through untouched.
  const splitThreadKey = (cid: string): { id: string; thread?: number } => {
    const m = /^(-?\d+)_t(\d+)$/.exec(cid)
    return m ? { id: m[1] as string, thread: Number(m[2]) } : { id: cid }
  }
  return {
    async sendMessage(chatId, text, opts) {
      const t = splitThreadKey(chatId)
      const other: Record<string, unknown> = {}
      if (t.thread !== undefined) other.message_thread_id = t.thread
      ...
      const sent = await bot.api.sendMessage(t.id, text, other)
    ...
    async editMessageText(chatId, messageId, text, opts) {
      ...
      await bot.api.editMessageText(t.id, messageId, text, other)
    },
    async setMessageReaction(chatId, messageId, emoji) {
      const t = splitThreadKey(chatId)
      await bot.api.setMessageReaction(t.id, messageId, [
        { type: 'emoji', emoji: emoji as ReactionTypeEmoji['emoji'] },
      ])
    },
    async sendChatAction(chatId, action) {
      const t = splitThreadKey(chatId)
      await bot.api.sendChatAction(t.id, action, t.thread !== undefined ? { message_thread_id: t.thread } : {})
    },
    async sendDocument(chatId, filePath, opts) {
      ...
      const sent = await bot.api.sendDocument(t.id, new InputFile(filePath), other)
      return { message_id: sent.message_id }
    },
    async sendPhoto(chatId, filePath, opts) {
      ...
      if (opts.caption !== undefined) other.caption = opts.caption
      const sent = await bot.api.sendPhoto(t.id, new InputFile(filePath), other)
      return { message_id: sent.message_id }
    },
    async deleteMessage(chatId, messageId) {
      const t = splitThreadKey(chatId)
      await bot.api.deleteMessage(t.id, messageId)
    },
    async editForumTopic(chatId, name) {
      const t = splitThreadKey(chatId)
      if (t.thread === undefined) return
      await bot.api.editForumTopic(t.id, t.thread, { name })
    },
    async closeForumTopic(chatId) {
      const t = splitThreadKey(chatId)
      if (t.thread === undefined) return
      await bot.api.closeForumTopic(t.id, t.thread)
    },
    async createForumTopic(chatId, name) {
      const t = splitThreadKey(chatId)
      const r = await bot.api.createForumTopic(t.id, name)
      return r.message_thread_id
    },
    async deleteForumTopic(chatId) {
      const t = splitThreadKey(chatId)
      if (t.thread === undefined) return
      await bot.api.deleteForumTopic(t.id, t.thread)
    },

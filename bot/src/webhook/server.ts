...
// answer. We do NOT cancel the underlying send — the owner still
...
// the owner sees the keyboard before the relay times out.
...
export interface WebhookDeps {
  ...
  // — so the reaction deterministically means "Agent-c read it through the
  ...
  // message so the owner still sees the answer. Wired through the same
  ...
  permissionUi?: {
    ...
    // Resolve from a non-Telegram surface (dashboard). Answers the relay AND
    // strips/edits the Telegram card so a web decision doesn't leave a stale
    // tappable card in Telegram. Optional — when absent we fall back to the
    // bare relay.answer() (card stays until its own timeout).
    resolveForWeb?(requestId: string, behavior: 'allow' | 'deny'): Promise<string>
  }
  // Allowlisted base chat ids (multichat policy allowlist.chats). When a
  // permission request carries a composite chat_id whose base is in this set,
  // the Allow/Deny card is routed to that topic; otherwise it falls back to
  // the owner DM. Absent → always owner DM (legacy behaviour).
  permissionAllowedChats?: string[]
  // model-switch (2026-07-30 per-topic extension): GET/POST /hooks/model/*.
  // Single ModelSwitch instance serves BOTH the master pane (its
  // constructor-bound paneTarget, used when the route's `pane` equals
  // masterPaneTarget) and any topic pane (via switchTo's per-call
  // TopicSwitchOverride) — one code path, so Telegram's /model and the
  // dashboard's buttons can never drift from each other. Optional so
  // deployments that never enable model_switch in config don't need to wire
  // this; absent → routes answer 503, matching every other optional
  // capability in this file.
  modelSwitchUi?: {
    masterPaneTarget: string
    modelSwitch: ModelSwitchLike
    // Path to the shared model-switch-state.json — see model-state.ts.
...
async function handleRequest(
  ...
  if (method === 'GET' && path === '/hooks/permission/pending') {
    await handlePermissionPending(req, res, deps, webhookToken)
    return
  }
  if (method === 'POST' && path === '/hooks/permission/decide') {
    await handlePermissionDecide(req, res, deps, webhookToken)
    return
  }
  ...
  // model-switch (2026-07-30): dashboard button proxy target, mirrors
  // /hooks/permission/*'s wiring exactly (same authGate, same optional-
  // capability-else-503 pattern). Wired before /hooks/agent for the same
  // more-specific-path-first reason as every other route above.
  if (method === 'GET' && path === '/hooks/model/status') {
    await handleModelStatus(req, res, deps, webhookToken)
    return
  }
  if (method === 'POST' && path === '/hooks/model/switch') {
    await handleModelSwitch(req, res, deps, webhookToken)
    return
  }
...
//
// chatId resolution for MVP (owner DM hardcoded):
...
//   is the owner's DM chat. TODO(multichat): derive from session_id
//   ⇨ tmux session ⇨ originating chat. Out of scope for MVP.
// ─────────────────────────────────────────────────────────────────────

function resolveAskChatId(config: AppConfig): string | undefined {
  // MVP: owner DM. The owner's chat_id == user_id in DM context.
...
// want only owner in permission_relay but a wider audience for
...
// forward the DM turn's final assistant text to the owner's Telegram when
...
// MILESTONE: dashboard surface for the permission gate — list pending requests
// and submit a decision. Loopback + bearer (authGate). Same relay the Telegram
// pgate:* taps drive, so a dashboard tap and a Telegram tap are interchangeable.
const PermissionDecideRouteSchema = z.object({
  requestId: z.string().min(1),
  behavior: z.enum(['allow', 'deny']),
})

async function handlePermissionPending(
  req: IncomingMessage,
  res: ServerResponse,
  deps: WebhookDeps,
  webhookToken: string | undefined,
  ...
  if (!authGate(req, res, webhookToken)) return
  const { permissionRelay } = deps
  if (!permissionRelay) {
    reply(res, 200, { pending: [] })
    return
  }
  const pending = permissionRelay
    .listPendingIds()
    .map((id) => permissionRelay.getPending(id))
...
async function handlePermissionDecide(
  req: IncomingMessage,
  res: ServerResponse,
  deps: WebhookDeps,
  webhookToken: string | undefined,
  ...
  if (!authGate(req, res, webhookToken)) return
  const { permissionRelay, permissionUi, log } = deps
  if (!permissionRelay) {
    reply(res, 503, { error: 'permission relay not wired' })
    return
  }
  const parsed = await readJsonBody(
    req,
    ...
    log,
    4096,
    PermissionDecideRouteSchema,
    'permission/decide',
  )
  if (!parsed.ok) return
  const { requestId, behavior } = parsed.value as {
    requestId: string
    behavior: 'allow' | 'deny'
  }
  // Prefer resolveForWeb so the Telegram card is edited (keyboard stripped +
  // verdict stamped) — otherwise a dashboard decision leaves a stale tappable
  // card hanging in Telegram. Fall back to the bare relay answer if the UI
  // (or the method) isn't wired.
  const status = permissionUi?.resolveForWeb
    ? await permissionUi.resolveForWeb(requestId, behavior)
    : permissionRelay.answer(requestId, behavior)
...
async function handlePermissionRequest(
  ...
  const { config, statePaths, log, permissionRelay, permissionUi, permissionAllowedChats } = deps
  ...
  const ownerChatId = allowed[0] === undefined ? undefined : String(allowed[0])
  ...
  // Destination of the Telegram card. Default = owner DM. If the request
  // carries a composite chat_id from a topic session AND its base chat is
  // allowlisted (owner user id or a multichat policy chat), route the card to
  // that topic instead — so per-topic permission prompts land in their topic.
  // Answer authz is unchanged: handlePgateCallback still gates on user_id.
  let chatId = ownerChatId
  const reqChatId = payload.chat_id
  if (reqChatId !== undefined && reqChatId !== '') {
    const baseChat = reqChatId.replace(/_t\d+$/, '')
    const trusted =
      baseChat === ownerChatId ||
      allowed.map(String).includes(baseChat) ||
      (permissionAllowedChats?.includes(baseChat) ?? false)
    if (trusted) {
      chatId = reqChatId
    } else {
      log.warn('permission/request chat_id base not allowlisted — routing card to owner DM', {
        base_chat: baseChat,
      })
    }
  }
  ...
  if (requestId !== undefined) {
    ...
    try {
      ...
      // If we can't deliver the keyboard the owner can never tap → there is
...
// ─────────────────────────────────────────────────────────────────────
// model-switch (2026-07-30 per-topic extension)
// ─────────────────────────────────────────────────────────────────────
...
// Defence in depth (2026-07-30, flagged in adversarial review): bearer auth
// alone gates WHO can call this route, not WHICH pane they can target — a
// leaked token (or a client-side typo) must not be able to `tmux
// respawn-pane -k` an arbitrary tmux session name on the same server. Same
// shape check /hooks/permission/request already applies to composite
// chat_ids (webhook/server.ts's own `baseChat = ...replace(/_t\d+$/, '')`
// pattern) — reused here rather than reinvented.
//
// Returns the resolved TopicSwitchOverride for a topic pane, or `null` for
// the master pane (paneTarget === masterPaneTarget), or `undefined` if
// `pane` is neither the master nor an allowlisted topic.
function resolveModelSwitchPane(
  pane: string,
  config: AppConfig,
  masterPaneTarget: string,
  permissionAllowedChats: string[] | undefined,
): { chatId: string } | null | undefined {
  if (pane === masterPaneTarget) return null
  const m = MULTICHAT_PANE_RE.exec(pane)
  if (!m || !m[1]) return undefined
  const chatId = m[1]
  const baseChat = chatId.replace(/_t\d+$/, '')
  const allowed = resolvePermissionGateAllowedUserIds(config)
  const trusted =
    allowed.map(String).includes(baseChat)
    || chatIdAllowed(config, baseChat)
    || (permissionAllowedChats?.includes(baseChat) ?? false)
...
async function handleModelStatus(
  ...
  if (!authGate(req, res, webhookToken)) return
  const { config, log, modelSwitchUi, permissionAllowedChats } = deps
  if (!modelSwitchUi) {
    reply(res, 503, { status: 'model_switch_unavailable' })
    return
  }
  const url = new URL(req.url ?? '/', 'http://localhost')
  const pane = url.searchParams.get('pane') ?? ''
  if (pane === '') {
    reply(res, 400, { error: 'missing pane' })
    return
  }
  const resolved = resolveModelSwitchPane(pane, config, modelSwitchUi.masterPaneTarget, permissionAllowedChats)
  if (resolved === undefined) {
    log.warn('model/status pane not allowlisted', { pane })
    reply(res, 403, { error: 'pane not allowlisted' })
    return
  }
  const entry = await getModelState(modelSwitchUi.stateFilePath, pane)
  reply(res, 200, { pane, target: entry?.target ?? 'unknown' })
}
...
async function handleModelSwitch(
  ...
  const parsed = await readJsonBody(
    ...
    res,
    log,
    MODEL_SWITCH_BODY_LIMIT_BYTES,
    ModelSwitchRouteSchema,
    'model/switch',
  )
  if (!parsed.ok) return
  const { pane, target } = parsed.value

  const resolved = resolveModelSwitchPane(pane, config, modelSwitchUi.masterPaneTarget, permissionAllowedChats)
  if (resolved === undefined) {
    log.warn('model/switch pane not allowlisted', { pane, target })
    reply(res, 403, { error: 'pane not allowlisted' })
    return
  }

  // Live pane required — respawn-pane -k has nothing to replace on a dead
  // pane (idle-killed topic, or one never spawned). Rather than silently
  // no-op, tell the caller plainly: switching a not-currently-running
  // topic isn't supported yet (cold spawn always boots plain Claude — see
  // model-state.ts's doc comment), so surfacing this loudly beats a button
  // that appears to work but didn't.
  const isAlive = await tmuxHasSession(pane)
  if (!isAlive) {
    reply(res, 409, {
      ok: false,
      error: 'pane not running — start a message in that topic first, then switch',
    })
    return
  }
  ...
  if (resolved === null) {
    result = await modelSwitchUi.modelSwitch.switchTo(target as ModelTarget)
  } else {
    try {
      const sessionId = await resolveTopicSessionId(resolved.chatId)
      result = await modelSwitchUi.modelSwitch.switchTo(target as ModelTarget, { paneTarget: pane, sessionId })
    } catch (err) {
      log.warn('model/switch topic session-id resolution failed', {
        pane,
        error: err instanceof Error ? err.message : String(err),
      })
      reply(res, 500, { ok: false, error: 'failed to resolve topic session id' })
      return
    }
  }

  if (result.ok) {
    await setModelState(modelSwitchUi.stateFilePath, pane, target as ModelTarget, log)
  }
  reply(res, result.ok ? 200 : 502, { ok: result.ok, target: result.target, ...(result.error ? { error: result.error } : {}) })
}

async function tmuxHasSession(paneTarget: string): Promise<boolean> {
  try {
    await execFileAsync('tmux', ['has-session', '-t', paneTarget], { timeout: 5000 })
    return true
  } catch {
    return false
  }
...
async function handleAskRequest(
  ...
  // dropped owner-facing context (Codex webhook #2).
  //
  ...
  if (requestId === undefined) {
    ...
    // succeeds the owner still sees the prompt and the answer flow

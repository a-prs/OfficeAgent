...
export interface AppendHotInput {
  ...
  // Header line re-written atop the file after an emergency trim. Defaults
  // to the recent.md (hot memory) header; the per-topic journal passes its
  // own so a trimmed topic journal isn't relabelled "Hot memory".
  header?: string
}
...
export async function appendHotEntry(
  ...
  await lockFor(input.path).run(async () => {
    ...
    const header = input.header ?? '# Hot memory -- last 24h rolling journal\n\n'

...
import {
  isTopicChatId,
  topicJournalPath,
  TOPIC_JOURNAL_HEADER,
  TOPIC_SHUTDOWN_MARKER,
} from '../router/topic-lifecycle.js'
...
export class MemoryWriter {
  ...
  // Persistent-topic journals (2026-06-18). When set, a Stop hook whose
  // chatId is a forum-topic composite is appended to a PER-TOPIC journal
  // ({topicJournalBase}/{compositeId}/journal.md) instead of the shared
  // recent.md — so each topic keeps its own rolling turn log for next-day
  // restore, and the master recent.md is not flooded with topic chatter.
  // `{topicJournalBase}` is the multichat chats dir ({workspaceDir}/chats).
  // Left undefined when multichat is off → every turn lands in recent.md
  // as before.
  private topicJournalBase: string | undefined = undefined
  ...
  /**
   * Wire the per-topic journal base ({workspaceDir}/chats). Called by
   * server.ts once the multichat router is up. Idempotent.
   */
  setTopicJournalBase(dir: string): void {
    this.topicJournalBase = dir
  }
  ...
  private async onStop(
    ...
    const isTopic =
      this.topicJournalBase !== undefined && isTopicChatId(chatId)
    if (isTopic) {
      // Per-topic journal (hot layer). Skip the internal idle-shutdown
      // directive — it is a system instruction the router injects to make
      // the dying session refresh its handoff, NOT a real chat turn, and
      // journaling it would pollute the very log it preserves.
      if (!userText.startsWith(TOPIC_SHUTDOWN_MARKER)) {
        await appendHotEntry({
          path: topicJournalPath(this.topicJournalBase as string, chatId),
          ts,
          agentLabel: this.cfg.agentLabel,
          sourceTag: this.cfg.sourceTag,
          // Wider snippets than recent.md: a topic journal is the restore
          // source, so it should carry enough of each turn to re-orient.
          userSnippet: snippet(userText, 300),
          agentSnippet: snippet(agentText || '(inline)', 800),
          maxBytes: this.cfg.maxHotBytes,
          trimKeepLines: this.cfg.trimKeepLines,
          header: TOPIC_JOURNAL_HEADER,
        })
      }
    } else {

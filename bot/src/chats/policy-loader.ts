...
export const ChatPolicySchema = z
  .object({
    ...
    // MILESTONE_03: dedicated bot-supergroup opt-in — respond to ALL
    // messages (no @mention needed); topics behave like DM threads.
    respond_to_all: z.boolean().default(false),
    // Forum supergroup: when true, the General topic (no "_t<threadId>"
    // suffix → chat_id === base) does NOT spawn a session. The router
    // fires a fixed script-side nudge ("use a Topic") with no LLM and
    // drops the message, so General stays a lobby and work happens in
    // dedicated topics (each its own isolated session).
    topics_only: z.boolean().default(false),
  })
  .strict()
 ...
 *     owner's DM always streams progress.

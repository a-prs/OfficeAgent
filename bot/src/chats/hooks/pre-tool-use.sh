#!/usr/bin/env bash
# Claude Code PreToolUse hook for multichat-agent-c.
#
...
# registered into the master Agent-c workspace via a stray settings.json) and
...
# Topic sessions carry a COMPOSITE chat id ("<base>_t<topicId>", e.g.
# "-100xxxxxxxxxx_t118") while policy.yaml is keyed by the BASE chat id.
# Try the composite first (allows a per-topic override if ever needed),
# then fall back to the base — mirrors tmux-session-pool.ts which does
# chatId.replace(/_t\d+$/, '') for the same lookup. Without this, deny
# rules under the base key never apply to topic sessions (fail-OPEN).
chats_cfg = policy.get('chats') or {}
base_chat_id = re.sub(r'_t\d+$', '', chat_id)
chat_cfg = chats_cfg.get(chat_id) or chats_cfg.get(base_chat_id) or {}

#!/usr/bin/env python3
"""Claude Code Stop hook for multichat-agent-c — bridge transcript → outbox.
...
Extraction algorithm:
    Tail-read the trailing ``TAIL_BYTES`` of the transcript and drop the first
    (possibly truncated) line when not starting at byte 0. Then walk lines
    backward collecting EVERY text-bearing assistant message of the current
    turn — across tool-use-only assistant messages and tool_result lines — and
    STOP at the last genuine user prompt. The collected messages are joined in
    chronological order (blank line between messages); the router chunks the
    result at 4000 chars.

    Why the whole turn, not just the last message: a turn that emits several
    text replies interleaved with tool calls (think -> say A -> tool -> say B)
    used to bubble only B, while A..(B-1) lived solely in the terminal mirror —
    so the chat saw a fragment, often a stray meta line (2026-06-19 prod
    report). Collecting the whole turn makes the bubble equal the terminal.
    This also subsumes the 2026-05-28 drop where a turn answered with text then
    ended on a tool call (the reply must not be lost just because the turn
    ended on a tool). We must NOT cross the user-prompt boundary: text from a
    PREVIOUS (already-delivered) turn would be a stale resend after a tool-only
    turn, a resume, or a clear. Dedupe (below) is the second guard against
    re-delivering the same turn.
...
_CHAT_ID_RE = re.compile(r"-?\d+(_t\d+)?")
...
# Topic-management markers (MILESTONE_03): [[topic: rename "Name"]] / [[topic: close]].
# The ROUTER executes them with the bot token; this hook only extracts intent.
_TOPIC_MARKER_RE = re.compile(r'\[\[topic:\s*(rename|close|create|delete)(?:\s+"([^"]*)")?\s*\]\]', re.IGNORECASE)
...
def read_last_assistant_text(transcript_path: Path) -> tuple[str, str | None] | None:
    """Tail-read the current turn's assistant text from a Claude transcript JSONL.

    Reads at most the trailing ``TAIL_BYTES`` bytes, drops the first possibly-
    truncated line when the read did not start at byte 0, then walks lines
    backward collecting EVERY text-bearing assistant message of the current
    ...
    delivered turns are never resurfaced). The collected messages are returned
    in chronological order, joined with blank lines, so the delivered text
    mirrors what the terminal showed for the whole turn — not just its last
    reply. The dedupe ``uuid`` is taken from the most recent text message.

    Args:
        transcript_path: Absolute path to the session transcript ``.jsonl``.

    Returns:
        ``(text, uuid)`` where ``text`` is every text-bearing assistant message
        of the current turn joined in order, and ``uuid`` is the most recent
        text message's ``uuid`` (or ``None`` if absent). Returns ``None`` if the
        file is missing/empty/unreadable, or the current turn produced no
        assistant text (pure tool-use turn).
    """
    try:
    ...
    # Collect EVERY text-bearing assistant message of the current turn (walking
    # backward to, but not across, the last genuine user prompt). The old rule
    # returned only the MOST RECENT text message, so when a turn emitted several
    # text replies interleaved with tool calls, only the last one reached the
    # bubble — everything said earlier lived solely in the terminal mirror
    # (2026-06-19 prod report). We now deliver the whole turn so the bubble
    # matches the terminal.
    collected: list[str] = []  # most-recent-first; reversed before join
    ...
    for line in reversed(lines):
        ...
        if not parts:
            ...
            # current turn; its sibling text messages are still collected.
            continue
        # The most recent text message (first seen walking backward) supplies
        # the dedupe uuid — it uniquely identifies this turn-end.
        if newest_uuid is None:
            uuid = obj.get("uuid")
            newest_uuid = uuid if isinstance(uuid, str) and uuid else None
        collected.append("\n".join(parts))

    if not collected:
        return None
    # collected is most-recent-first; reverse to chronological order so the
    # delivered text matches the on-screen terminal sequence. Separate messages
    # are joined with a blank line; the router chunks the result at 4000 chars.
    collected.reverse()
    return "\n\n".join(collected), newest_uuid
...
def main() -> int:
    ...
    # Topic-management markers -> structured actions for the router.
    topic_actions = []
    for _op, _name in _TOPIC_MARKER_RE.findall(assistant_text):
        _a = {"op": _op.lower()}
        if _name.strip():
            _a["name"] = _name.strip()
        topic_actions.append(_a)
    if topic_actions:
        assistant_text = _TOPIC_MARKER_RE.sub("", assistant_text).strip()

    # Nothing to deliver: neither visible text nor any attachment.
    if not assistant_text.strip() and not attach_files and not topic_actions:
    ...
    if topic_actions:
        message["topic_actions"] = topic_actions

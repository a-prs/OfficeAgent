...
export function renderMediaDescriptor(media: MediaDescriptor): string {
  ...
  switch (media.kind) {
    ...
    case 'document': {
      ...
      parts.push(attr('size', media.size))
      // local_path lets per-chat/topic sessions (which lack the
      // download_attachment MCP tool) Read the file directly; the master DM
      // session benefits too (no tool round-trip needed).
      parts.push(attr('local_path', media.localPath))
      break
    }
    case 'voice': {
    ...
    case 'audio': {
      ...
      parts.push(attr('size', media.size))
      parts.push(attr('duration_sec', media.durationSec))
      // local_path (when the file fit the 20MB Bot API cap) lets per-chat/topic
      // sessions Read the bytes directly — they lack the download_attachment tool.
      parts.push(attr('local_path', media.localPath))
      break
    }
    case 'video': {

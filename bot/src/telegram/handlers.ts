...
export interface HandlerDeps {
  ...
  // Path to the shared model-switch state JSON (dashboard/Telegram sync).
  // MUST be the exact same constant server.ts computes for itself
  // (`join(statePaths.root, 'model-switch-state.json')`) — threaded rather
  // than re-derived here so there's a single source of truth (2026-07-30
  // adversarial review finding: a second, independently-derived path risked
  // drifting from server.ts's if the filename ever changed there).
...
// group message — the owner and other speakers would see the bot
...
async function gateAndNotify(
  ...
  // (typically just the owner) may summon the bot via @-mention or
  ...
  if (deps.policy && input.chatType !== 'private') {
    // MILESTONE_03: a dedicated bot-supergroup can opt into respond-to-all
    // (no @mention) via policy.chats[id].respond_to_all — topics there behave
    // like DM threads. The router still enforces allowlist.users downstream.
    const respondToAll = deps.policy.chats[decision.chatId]?.respond_to_all === true
    const addressed = respondToAll || isAddressedToBot(ctx, deps.policy.mention_allowlist)
    if (!addressed) {
  ...
  // land in the master (channel-agent-c) session — the owner's main
  ...
  // MILESTONE_03 topic-session: a forum-topic message carries
  // is_topic_message + message_thread_id. Route it to its own per-topic
  // tmux session (even in a private DM — Telegram now supports topics in
  // DMs). The General topic has no thread id and stays in the master session.
  const topicThreadId =
    ctx.message?.message_thread_id !== undefined
      ? ctx.message.message_thread_id
      : undefined
  if (deps.router && deps.policy && (isGroup || topicThreadId !== undefined)) {
    ...
    const inboundMsg: InboundMessage = {
      text: buildText(),
      chat_id:
        topicThreadId !== undefined
          ? `${decision.chatId}_t${topicThreadId}`
          : decision.chatId,
      user_id: decision.senderId,
      user:
      ...
      // Forward the rendered media descriptors (voice transcript, document
      // metadata, photo local_path) so the topic session sees the same
      // <media .../> tags the master DM session would — without this a
      // voice message arrives as empty text and the transcript is lost.
...
export async function sendAlbumNotification(
  ...
  // legacy notify path below, landing in the master (channel-agent-c)
  ...
  if (deps.router && deps.policy && isGroup) {
    ...
    const inboundMsg: InboundMessage = {
      ...
      // Same fix as the single-message path: carry the rendered descriptors so
      // a topic-routed album does not lose voice transcripts / attachment meta.
      ...(combinedDescriptors.length > 0 ? { media_descriptors: combinedDescriptors } : {}),
...
export async function handleInboundText(ctx: Context, deps: HandlerDeps): Promise<void> {
  ...
  // update", not "Agent-c read it" — if a turn was busy, the message queued
  ...
  // owner's intent was lost. SECURITY gate: live permission verdict
  ...
  // enforces:
  //   - approver allowlist (only the owner may answer)
  ...
  // OOB short-circuit: when the sender is allowed AND (the chat is a DM, OR
  // it's a `/model` command typed directly inside a forum topic) AND the
  // text parses as a known /command, handle it inline and DO NOT fall
  ...
  // DM already requires (allowed_user_ids + allowed_chat_ids) — this isn't
  // a new security bar, it's the identical one `resolveModelSwitchPane`/
  // `chatIdAllowed` in webhook/server.ts already treat as sufficient for
  // pane-target trust elsewhere.
  ...
  if (parsed) {
    ...
    const chatId =
      chatNum !== undefined
        ...
        : undefined
    if (
      (chatType === 'private' || isTopicModelCommand)
      && chatId
      && senderId
      && allowedSender
      && allowedChat
...
// ─────────────────────────────────────────────────────────────────────
// Document — downloaded eagerly to the inbox (like photos) so the descriptor
// carries local_path. Per-chat/topic sessions lack the download_attachment
// MCP tool, so without a local path a document in a topic arrives as bare
// metadata the session cannot open. The 20MB Bot API cap applies; on failure
// the descriptor is still emitted (sans local_path) and the master DM session
// can fall back to the download_attachment tool.
// ─────────────────────────────────────────────────────────────────────
...
export async function handleInboundAudio(ctx: Context, deps: HandlerDeps): Promise<void> {
  ...
  const buildAudio = async (): Promise<MediaDescriptor[]> => {
    ...
    if (!audio) return []
    // Eager download to the inbox (like photos/documents) so the descriptor
    // carries local_path: per-chat/topic sessions lack the download_attachment
    // MCP tool, so without a local path an audio file in a topic is unreachable.
    // Over the 20MB Bot API cap → downloadPhotoToInbox returns undefined and we
    // ship metadata only (the master DM session can still use download_attachment).
    const localPath = await downloadPhotoToInbox(
      deps.botApi,
      deps.botToken,
      audio.file_id,
...
export async function handleInboundVideo(ctx: Context, deps: HandlerDeps): Promise<void> {
  ...
  const buildVideo = async (): Promise<MediaDescriptor[]> => {
    ...
    // Eager download to the inbox (like photos/documents) so the descriptor
    // carries local_path: per-chat/topic sessions lack the download_attachment
    // MCP tool, so without a local path a video in a topic is unreachable.
    // Over the 20MB Bot API cap → downloadPhotoToInbox returns undefined and we
    // ship metadata only (the master DM session can still use download_attachment).
    const localPath = await downloadPhotoToInbox(
      deps.botApi,
...
export async function handleInboundVideoNote(ctx: Context, deps: HandlerDeps): Promise<void> {
  ...
  await gateAndNotify(
    ...
    async () => {
      ...
      if (!note) return []
      // Eager download (like photos/documents) so the descriptor carries
      // local_path for per-chat/topic sessions (no download_attachment tool).
      // Over the 20MB cap → undefined → metadata only.
      const localPath = await downloadPhotoToInbox(
        deps.botApi,
        deps.botToken,
        note.file_id,

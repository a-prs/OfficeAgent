...
// dispatches `ask:*` callback_query payloads, and feeds the owner's
...
//   1. Owner taps «Другое» → we send «Введи ответ текстом» with
//      `force_reply: true, selective: true` (Telegram clients auto-quote
//      the prompt for the owner). The returned message_id is stored
...
//      blocking the owner's intent and silently consuming sensitive
...
export interface AskUserQuestionUi {
   ...
   * the owner's UI doesn't appear stuck.
   */
...
export function createAskUserQuestionUi(
  ...
  // the anchor message no longer exists (owner deleted it, 48h edit
    ...
    try {
      ...
      switch (cls.kind) {
        case 'benign':
          ...
          return
        case 'message_gone':
          // Owner deleted the keyboard message, or it aged out of the
  ...
  async function advanceAfterAnswer(requestId: string, prevMessageId: number | undefined, prevChatId: string | undefined): Promise<void> {
    ...
    if (stillPending) {
      // Clear the previous keyboard so the owner can't double-answer
  ...
  async function handleAskCallback(ctx: AskCallbackContext): Promise<void> {
    ...
    try {
      ...
      // Telegram clients auto-quote the prompt — the owner's next
      ...
      // Best-effort spinner ack so the owner's UI doesn't hang.
      await ctx.answerCallbackQuery().catch(() => {})
    }
  }
  ...
  async function tryHandleOtherText(input: {
    ...
    // and the owner can still answer by tapping the reply UI.
    ...
    // Only the owner (or an allowed approver) can complete the

...
// confirm and dispatches the owner's tap back into the relay.
//
...
} from '../channel/permission-gate-relay.js'
...
export interface PermissionGateUi {
  ...
  /** Resolve a request from a NON-Telegram surface (e.g. the dashboard's
   *  Allow/Deny). Answers the relay AND edits the Telegram card to strip its
   *  keyboard + show the verdict — so a decision taken in the web UI doesn't
   *  leave a stale, tappable card hanging in Telegram (and vice-versa the
   *  Telegram path already edits, so both surfaces stay in sync). */
  resolveForWeb(requestId: string, behavior: 'allow' | 'deny'): Promise<PermissionGateStatus>
}
...
export function createPermissionGateUi(deps: PermissionGateUiDeps): PermissionGateUi {
  ...
  async function handlePgateCallback(ctx: PgateCallbackContext): Promise<boolean> {
    ...
    // buttons can't be tapped again. Shared with resolveForWeb so a decision
    // taken on the dashboard closes the Telegram card identically.
    await closeCardWithVerdict(pendingBefore, parsed.behavior)
  ...
  // Strip the keyboard and stamp the verdict onto a resolved card. No-op when
  // the request never got a Telegram card (pass_through) or already lost it.
  async function closeCardWithVerdict(
    pendingBefore: Readonly<PendingPermissionGate> | undefined,
    behavior: 'allow' | 'deny',
  ): Promise<void> {
    if (!pendingBefore?.chatId || pendingBefore.telegramMessageId === undefined) return
    const verdict = behavior === 'allow' ? '✅ Разрешено' : '❌ Запрещено'
    ...
    await telegramApi
      ...
      .catch((err) => {
        log.warn('permission_gate edit after decision failed', {
          request_id: pendingBefore.requestId,
          error: err instanceof Error ? err.message : String(err),
        })
      })
  }

  // Dashboard (or any non-Telegram surface) resolves a request: snapshot the
  // pending card BEFORE answering (answer() settles + drops it from the map),
  // resolve, then close the Telegram card. First decision wins — a later tap in
  // Telegram on the same card returns 'idempotent' and is a no-op.
  async function resolveForWeb(
    requestId: string,
    behavior: 'allow' | 'deny',
  ): Promise<PermissionGateStatus> {
    const pendingBefore = relay.getPending(requestId)
    const status = relay.answer(requestId, behavior)
    if (status === 'allow' || status === 'deny') {
      await closeCardWithVerdict(pendingBefore, behavior)
    }
    return status
  }

  return { sendPrompt, handlePgateCallback, clearKeyboard, resolveForWeb }
}

...
export const AppConfigSchema = z.object({
  ...
  status: z.object({
    ...
    // Owner request 2026-05-27: the bare «Печатает...» bubble is visual
  ...
  // list, older ones collapse to «+M завершено ранее» — the owner wants
  ...
  // InboundWatcher (PR-A3, 2026-05-20) — auto-reply «Бот занят» when the
  // owner sends plain text while a Claude session is mid-tool. Debounced
  ...
  // the owner can answer from his phone while a session runs headless.
  //
  ...
  // deliberately do NOT copy the default here: duplicating the owner
  ...
  // back to its native flow. 5 min matches typical owner response
  ...
  // owner's bypassPermissions DM session. The PreToolUse hook
  ...
  // the hook + restart channel-agent-c). While off, the HTTP route answers
  ...
  // to deny — 10 min gives the operator time to tap from phone/web without the
  // prompt expiring mid-task (raised from 2 min per Andrey, 2026-06-17).
  //

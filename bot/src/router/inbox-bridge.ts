...
export type InboundMessage = {
  ...
  media_paths?: string[]
  // Rendered `<media kind="..." ... />` descriptor tags (one per attachment),
  // identical to what the master DM session receives via buildChannelContent's
  // `mediaDescriptors`. This is the ONLY carrier of voice transcripts and
  // non-photo attachment metadata for per-chat/topic sessions: `media_paths`
  // only ever holds downloaded photo paths, so without this a voice message in
  // a topic would arrive as empty text (transcript silently lost). The
  // entrypoint prefers these over `media_paths` when present (the photo
  // descriptor already carries local_path).
  ...
  message_id?: string
  // Internal maintenance directive (2026-07-06): when set to 'raw_command',
  // the entrypoint watcher submits `text` to the pane VERBATIM — no
  // "[from @user]" wrapping, no reply_context/media handling — and, once
  // the composer settles back to idle, touches `ack_path` itself (the
  // watcher, not the agent — a CLI-level slash command like `/compact`
  // involves no agent turn/tool-calls to hang an ack off of). Never used
  // for real chat content; omitted entirely for normal messages.
  kind?: 'raw_command'
  ack_path?: string
}
...
export const OutboxMessageSchema = z
  .object({
    ...
    topic_actions: z
      .array(z.object({ op: z.enum(['rename', 'close', 'create', 'delete']), name: z.string().optional() }))
      .optional(),
  })
  .refine(
    (m) => (m.text !== undefined && m.text.length > 0) || (m.files !== undefined && m.files.length > 0) || (m.topic_actions !== undefined && m.topic_actions.length > 0),

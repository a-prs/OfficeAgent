...
// Widened again 2026-08-22 (Андрей): 300s still wasn't enough for /compact
// on a genuinely large transcript in real use — bumped to 420s (7 min),
// with the outer cap raised to keep the same ~120s margin over the new sum.
...
export interface MultichatTelegramApi {
  ...
  deleteMessage?: TelegramApi['deleteMessage']
  ...
  editForumTopic?: TelegramApi['editForumTopic']
  closeForumTopic?: TelegramApi['closeForumTopic']
  createForumTopic?: TelegramApi['createForumTopic']
  deleteForumTopic?: TelegramApi['deleteForumTopic']
}
...
// The ONLY hard limit on multichat attachments (owner 2026-06-10: «только
...
// topics_only: fixed, script-side reply fired when a message lands in a forum
// supergroup's General topic. No LLM, no session — pure transport. Throttled
// per chat so a burst of General messages produces at most one nudge per window.
const GENERAL_NUDGE_THROTTLE_MS = 60_000
const GENERAL_NUDGE_TEXT =
  '⚠️ Это General — здесь всё вперемешку и без отдельной сессии. ' +
  'Открой или создай конкретный Topic и пиши туда: у каждого топика своя изолированная сессия.'
...
export class MultichatRouter {
  ...
  >()
  // topics_only: throttle the "use a Topic" nudge so a burst into General
  // does not spam one reply per message. Keyed by base chat id → last send ms.
  private readonly generalNudgeAt = new Map<string, number>()
  ...
  constructor(deps: RouterDeps) {
    ...
    this.onSessionKilled = deps.onSessionKilled
    // MILESTONE_03: tear down a chat's mirror when its session dies
    // (idle watchdog or explicit kill).
    this.pool.onKill = (cid: string): void => {
      this.stopMirror(cid)
      this.onSessionKilled?.(cid)
    }
    // Persistent topics (2026-06-18): give a topic session a chance to dump
    // its handoff before the idle watchdog kills it.
    this.pool.onBeforeKill = (cid: string): Promise<void> =>
      this.gracefulTopicDump(cid)
  ...
  /**
   * Graceful pre-kill handoff dump (persistent topics, 2026-06-18).
   *
   * Invoked by the pool's idle watchdog just before a topic session is
   * killed. We inject an INTERNAL directive (opened with
   * TOPIC_SHUTDOWN_MARKER so MemoryWriter skips journaling it) telling the
   * still-alive session to rewrite its handoff.md and touch an ack
   * sentinel, then poll for the ack up to GRACEFUL_DUMP_POLL_MS.
   *
   * This only adds a CURATED summary on top of the per-topic journal, which
   * already captured every turn — so every failure path (inject error,
   * timeout, unresponsive session) degrades cleanly to "journal only" and
   * never blocks the reap (the pool also caps us at BEFORE_KILL_TIMEOUT_MS).
   *
   * Non-topic chats (the master DM) are a no-op: their durable memory is
   * the master workspace's recent.md/handoff, written elsewhere.
   */
  private async gracefulTopicDump(chatId: string): Promise<void> {
    if (!isTopicChatId(chatId)) return
    const handoffPath = topicHandoffPath(this.getChatsBasePath(), chatId)
    const ackPath = shutdownAckPath(this.stateDir, chatId)
    // Clear any stale ack from a prior dump so we never observe a false hit.
    await rm(ackPath, { force: true }).catch(() => {})

    const directive: InboundMessage = {
      text:
        `${TOPIC_SHUTDOWN_MARKER} Сессия этого топик-проекта сейчас закроется ` +
        `по неактивности. Это ВНУТРЕННЯЯ системная команда — НЕ отвечай в чат ` +
        `и не вызывай reply.\n\n` +
        `1) Перезапиши файл ${handoffPath}: что за проект, что сделано, где ` +
        `остановились, открытые вопросы, конкретные следующие шаги — коротко, ` +
        `чтобы следующая сессия продолжила без потери контекста.\n` +
        `2) Затем выполни: touch ${ackPath}\n` +
        `После этого завершай ход, ничего не отправляя пользователю.`,
      chat_id: chatId,
      user_id: 'system',
      user: 'system',
    ...
    try {
      await writeToInbox(chatId, directive, this.stateDir)
    } catch (err) {
      this.logger.warn('router.graceful_dump.inject_failed', {
        chat_id: chatId,
    ...
    const deadline = Date.now() + GRACEFUL_DUMP_POLL_MS
    let acked = false
    while (Date.now() < deadline) {
      ...
      await sleep(GRACEFUL_DUMP_POLL_INTERVAL_MS)
    }
    if (acked) {
      this.logger.info('router.graceful_dump.ok', { chat_id: chatId })
    } else {
      this.logger.warn('router.graceful_dump.timeout', {
        chat_id: chatId,
        note: 'handoff not confirmed before kill; journal preserves the turns',
      })
    }
    // Sentinel is operational state; clear it (kill() scrubs queue dirs but
    // not this root-level marker).
    await rm(ackPath, { force: true }).catch(() => {})

    await this.gracefulPreKillCompact(chatId)
  }

  /**
   * Pre-kill transcript compaction (2026-07-06).
   *
   * gracefulTopicDump (above) only refreshes the CURATED handoff.md summary
   * — it never touches the resumable transcript itself. `claude --resume`
   * reloads that transcript verbatim on the topic's next spawn, so a
   * session that grew large over a long working stretch used to pay for
   * its own compaction lazily, on the operator's first message back — a
   * visible stall with nothing to show for it while it happens.
   *
   * We submit `/compact` directly into the still-alive pane instead, as a
   * `kind: 'raw_command'` directive (bypasses the normal "[from @user]"
   * chat-message wrapping — a CLI slash command needs the composer to
   * contain exactly `/compact`, nothing else). The entrypoint watcher
   * touches the ack itself once the composer settles back to idle, since a
   * slash command involves no agent turn/tool-call to hang an ack off of.
   *
   * Same failure posture as the handoff dump: best-effort, bounded, and a
   * timeout just means the NEXT resume compacts lazily as it did before —
   * never worse than the pre-existing behaviour.
   */
  private async gracefulPreKillCompact(chatId: string): Promise<void> {
    if (!isTopicChatId(chatId)) return
    const ackPath = compactAckPath(this.stateDir, chatId)
    await rm(ackPath, { force: true }).catch(() => {})
    ...
    try {
      ...
      this.logger.warn('router.graceful_dump.compact_inject_failed', {
        chat_id: chatId,
        error: err instanceof Error ? err.message : String(err),
      })
      return
    ...
    const deadline = Date.now() + COMPACT_POLL_MS
    let acked = false
    while (Date.now() < deadline) {
      if (existsSync(ackPath)) {
        acked = true
    ...
    if (acked) {
      this.logger.info('router.graceful_dump.compact_ok', { chat_id: chatId })
    } else {
      this.logger.warn('router.graceful_dump.compact_timeout', {
        chat_id: chatId,
        note: 'transcript not confirmed compacted before kill; next resume compacts lazily instead',
      })
    }
    await rm(ackPath, { force: true }).catch(() => {})
  }
  ...
  async dispatch(input: InboundMessage): Promise<void> {
    ...
    // MILESTONE_03 topic-session: policy/allowlist are keyed by the BASE
    // chat id; the "_t<threadId>" suffix only scopes the per-topic session.
    const baseChatId = input.chat_id.includes('_t')
      ? input.chat_id.slice(0, input.chat_id.indexOf('_t'))
      : input.chat_id
    const chatPolicy = getChatPolicyOrDeny(this.policy, baseChatId)
    const chatAllowed = this.policy.allowlist.chats.includes(baseChatId)
    ...
    // 2b. topics_only — General lobby guard. In a forum supergroup the
    //     General topic carries no "_t<threadId>" suffix, so its chat_id
    //     equals the base. When the chat is topics_only we never spawn a
    //     session for General: fire a fixed, script-side nudge (no LLM) and
    //     drop the message. Throttled so a burst can't spam the reply.
    if (chatPolicy.topics_only && input.chat_id === baseChatId) {
      const now = Date.now()
      const last = this.generalNudgeAt.get(baseChatId) ?? 0
      if (now - last >= GENERAL_NUDGE_THROTTLE_MS) {
        this.generalNudgeAt.set(baseChatId, now)
        const opts: { reply_to_message_id?: number } = {}
        const parsed =
          input.message_id !== undefined ? Number(input.message_id) : Number.NaN
        if (Number.isInteger(parsed)) opts.reply_to_message_id = parsed
        try {
          ...
          this.logger.warn('router.dispatch.general_nudge_failed', {
            chat_id: input.chat_id,
            error: err instanceof Error ? err.message : String(err),
          })
        }
      }
      this.logger.debug?.('router.dispatch.general_dropped', {
        chat_id: input.chat_id,
        user_id: input.user_id,
      })
      return
    ...
    this.ensureMirror(input.chat_id)
    this.bumpMirror(input.chat_id)
  ...
  private async deliverClaim(
    ...
    // MILESTONE_03: forum-topic actions the agent requested via [[topic: …]]
    // markers. Best-effort, AFTER text/files. The bot token lives here, never
    // in the session. chatId is composite; the API wrapper decodes the thread.
    if (message.topic_actions !== undefined && message.topic_actions.length > 0) {
      for (const act of message.topic_actions) {
        try {
          if (act.op === 'rename' && act.name !== undefined && act.name.length > 0) {
            await this.telegramApi.editForumTopic?.(chatId, act.name)
          } else if (act.op === 'close') {
            this.stopMirror(chatId)
            await this.telegramApi.closeForumTopic?.(chatId)
            await this.pool.kill(chatId)
          } else if (act.op === 'create' && act.name !== undefined && act.name.length > 0) {
            await this.telegramApi.createForumTopic?.(chatId, act.name)
          } else if (act.op === 'delete') {
            this.stopMirror(chatId)
            await this.telegramApi.deleteForumTopic?.(chatId)
            await this.pool.kill(chatId)
          }
        } catch (err) {
          this.logger.warn('router.outbox.topic_action_failed', {
            chat_id: chatId, op: act.op,
            error: err instanceof Error ? err.message : String(err),
          })
        }
      }
   ...
   * owner the ONLY hard limit is the secret denylist — any other file (any

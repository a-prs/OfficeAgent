...
//
// GBRAIN_BEARER — forwarded so per-topic Claude sessions can reach
// gbrain HTTP MCP servers (recall/memory/task/swarm). This is the
// only gbrain credential that crosses the boundary: it accesses
// localhost-only services and is not an escalation path for a
// compromised topic session (those services have no write-access to
...
const FORBIDDEN_ENV_REGEX =
  /^(?:GBRAIN_(?!BEARER$)|OPENAI_|ANTHROPIC_|TELEGRAM_|AWS_|SUPABASE_|STRIPE_|GITHUB_|NPM_).*$|^.*(?:^|_)(?:TOKEN|API_KEY|SECRET|PASSWORD|PRIVATE_KEY)$|^.+_URL$/
...
export function buildSanitizedTmuxEnv(parentEnv: NodeJS.ProcessEnv): {
  ...
  // Permission-gate hook (PreToolUse) in a bypass topic session needs the
  // loopback webhook token + URL to render interactive Allow/Deny in Telegram.
  // These are 127.0.0.1-only (NOT the bot token) — useless if exfiltrated
  // off-host. Narrow, explicit re-export AFTER the forbidden filter: ONLY
  // these two keys stay exempt; every other TELEGRAM_*/credential key remains
  // blocked by FORBIDDEN_ENV_REGEX above.
  // TELEGRAM_WEBHOOK_PORT is the loopback port-allowlist hint the gate hook
  // checks before shipping the token (non-prod ports like 8090 are not in the
  // hard whitelist), so it rides along with the token + URL.
  // CLAUDE_CODE_OAUTH_TOKEN: long-lived OAuth token for Claude Code auth.
  // Topic sessions have no .credentials.json and no ANTHROPIC_API_KEY —
...
// Upper bound on the graceful pre-kill hook (handoff dump + transcript
...
// Widened 2026-08-20, then again 2026-08-22 alongside GRACEFUL_DUMP_POLL_MS/
// COMPACT_POLL_MS (multichat-router.ts): those two budgets now sum to 600s,
// so this cap must stay above that with headroom, or it forcibly kills the
// pane mid-write on the slow path this change exists to fix.
const BEFORE_KILL_TIMEOUT_MS = 720_000
...
export class TmuxSessionPool {
  ...
  // MILESTONE_03: invoked after a session is killed (idle watchdog or
  // explicit kill) so the router can stop the chat's tmux mirror.
  onKill: ((chatId: string) => void) | undefined = undefined
  // Persistent topics (2026-06-18): invoked by the idle watchdog BEFORE a
  // session is killed, so the router can ask a topic session to dump its
  // handoff while it is still alive. Best-effort: bounded by
  // BEFORE_KILL_TIMEOUT_MS and never allowed to throw/block the reap (the
  // per-topic journal already preserves the turns). Explicit kill() callers
  // (topic close/delete) bypass this.
  onBeforeKill: ((chatId: string) => Promise<void>) | undefined = undefined
  ...
  // chatIds currently in the graceful-dump-then-kill sequence. Guards
  // against a second watchdog tick (the interval keeps firing while a dump
  // is in flight) re-reaping a session that is already being torn down.
  ...
  constructor(opts: TmuxSessionPoolOptions) {
    ...
    // hooks registration. Default mirrors the canonical Agent-c layout
  ...
  async kill(chatId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.warn('pool.onKill callback failed', {
        chatId,
        error: err instanceof Error ? err.message : String(err),
      })
    }
  ...
  async runIdleCheck(): Promise<void> {
    ...
    for (const [chatId, handle] of this.sessions) {
      const chatPolicy = this.policy.chats[chatId.replace(/_t\d+$/, '')]
    ...
    for (const chatId of toKill) {
      // Skip if a prior (still-running) tick already owns this reap.
      if (this.reaping.has(chatId)) continue
      this.reaping.add(chatId)
      try {
        // Race guard (2026-07-07): the snapshot above can go stale — a
        // fresh inbound message may `touch()` this chat between the
        // snapshot and now, or during the multi-minute pre-kill hook
        // below. Re-validate idle against live state at both checkpoints
        // so a session that just woke up doesn't eat a forced /compact +
        // kill moments after the operator wrote to it (observed 2026-07-06:
        // topic t3602 got hit with the pre-kill /compact right as a new
        // message arrived, because nothing re-checked freshness once the
        // reap was already queued).
        if (!this.isStillIdle(chatId)) {
          this.logger.info('tmux session idle-kill aborted: message arrived before reap started', {
            chatId,
          })
          continue
        }
        await this.runBeforeKill(chatId)
        if (!this.isStillIdle(chatId)) {
          this.logger.info('tmux session idle-kill aborted: message arrived during pre-kill hook', {
            chatId,
          })
          continue
        }
        ...
        this.reaping.delete(chatId)
      }
    }
  }

  /** Live re-check of the same idle_ttl_ms rule runIdleCheck's snapshot used. */
  private isStillIdle(chatId: string): boolean {
    const handle = this.sessions.get(chatId)
    if (handle === undefined) return false
    const chatPolicy = this.policy.chats[chatId.replace(/_t\d+$/, '')]
    const ttl = chatPolicy?.idle_ttl_ms ?? 1_800_000
    return Date.now() - handle.lastMessageAt > ttl
  }

  /**
   * Best-effort graceful pre-kill hook. Gives a topic session a bounded
   * window to dump its handoff before we kill the tmux session. A hung or
   * unresponsive session can NOT stall the watchdog: the call races a
   * BEFORE_KILL_TIMEOUT_MS timer and any throw is swallowed (the per-topic
   * journal is the durable fallback, so a missed dump only loses the
   * curated summary, never the turns).
   */
  private async runBeforeKill(chatId: string): Promise<void> {
    if (this.onBeforeKill === undefined) return
    let timer: ReturnType<typeof setTimeout> | undefined
    try {
      await Promise.race([
        this.onBeforeKill(chatId),
        new Promise<void>((resolve) => {
          timer = setTimeout(resolve, BEFORE_KILL_TIMEOUT_MS)
          timer.unref?.()
        }),
      ])
    } catch (err) {
      this.logger.warn('pool.onBeforeKill failed', {
        chatId,
        error: err instanceof Error ? err.message : String(err),
      })
    } finally {

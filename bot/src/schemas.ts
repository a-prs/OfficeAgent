...
// plugin sends an Allow/Deny keyboard to the owner and long-waits for the
...
export const PermissionRequestRouteSchema = z.object({
  ...
  // Composite chat id of the requesting session ("<chatId>_t<threadId>" for a
  // topic session). Lets the gate route the Telegram card to the originating
  // topic instead of the owner DM. Validated against the allowlist before use.

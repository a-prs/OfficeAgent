...
// from Bot API with retry_after ≈ 300s — long enough that the owner lost
...
export function createRateLimitedTelegramApi(
  ...
  return {
    ...
    async editForumTopic(chatId: string, name: string): Promise<void> {
      return withRetry('editForumTopic', () => raw.editForumTopic(chatId, name))
    },
    async closeForumTopic(chatId: string): Promise<void> {
      return withRetry('closeForumTopic', () => raw.closeForumTopic(chatId))
    },
    async createForumTopic(chatId: string, name: string): Promise<number> {
      return withRetry('createForumTopic', () => raw.createForumTopic(chatId, name))
    },
    async deleteForumTopic(chatId: string): Promise<void> {
      return withRetry('deleteForumTopic', () => raw.deleteForumTopic(chatId))
    },
  }
}

...
describe('redactSecrets — URL exemption for the generic long-token rule', () => {
  ...
  // (`office2-plugin-claude-code` → `dash***code`), producing dead URLs the
  // owner could not open. Long path segments inside http(s) URLs are
  ...
  test('GitHub PR link with a long repo slug survives intact', () => {
    const url = 'https://github.com/a-prs/office2-plugin-claude-code/pull/49'
    expect(redactSecrets(`PR готов: ${url}`)).toBe(`PR готов: ${url}`)
  })

  test('commit-SHA URL survives intact', () => {
    const url =
      'https://github.com/a-prs/office2-plugin-claude-code/commit/' +
      'a'.repeat(40)
  ...
  test('a kebab-case slug OUTSIDE a URL is now left intact (hyphen exemption)', () => {
    ...
    const out = redactSecrets(
      'slug office2-plugin-claude-code and https://github.com/a-prs/office2-plugin-claude-code',
    )
    expect(out).toContain('slug office2-plugin-claude-code and')
    expect(out).toContain(
      'https://github.com/a-prs/office2-plugin-claude-code',
    )
  })
  ...
  test('GitHub PAT inside a URL is STILL redacted', () => {
    const out = redactSecrets(
      `https://ghp_${'Z'.repeat(36)}@github.com/a-prs/repo.git`,
    )
  ...
  test('URL exemption is idempotent', () => {
    const input =
      'see https://github.com/a-prs/office2-plugin-claude-code/pull/49 and token abcd1234567890efghij5678WXYZ'
  ...
  test('trailing punctuation does not extend the exemption', () => {
    ...
    const out = redactSecrets(
      'https://github.com/a-prs/office2-plugin-claude-code/pull/49. Token abcd1234567890efghij5678WXYZ',
    )
    expect(out).toContain('office2-plugin-claude-code/pull/49.')
  ...
  test('URL-borne secret redaction is idempotent', () => {
    ...
    const input = [
      ...
      'https://github.com/a-prs/office2-plugin-claude-code/pull/49,SECRET_0123456789012345678901234567',
    ].join(' ')
    const once = redactSecrets(input)
    const twice = redactSecrets(once)
    expect(twice).toBe(once)

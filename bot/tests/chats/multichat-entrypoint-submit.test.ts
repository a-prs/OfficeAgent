...
describe('submit_prompt', () => {
  ...
  test('cyrillic fingerprint survives 60-char truncation (UTF-8 safe)', () => {
    installStubTmux()
    const ru =
      '[from @office2eshiev] Проверь пожалуйста последние коммиты плагина и скажи решили ли мы все фиксы по репорту ученика'
    writeFileSync(join(dir, 'pane-1.txt'), READY_PANE)
...
describe('process_inbox', () => {
  ...
  test('confirmed submit moves file to .processed/ unprefixed', () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ text: 'a normal message that submits', user: 'office2' }),
    )

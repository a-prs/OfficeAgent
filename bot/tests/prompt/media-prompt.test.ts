...
describe('renderMediaDescriptor', () => {
  ...
  test('builds video tag with local_path (eager download)', () => {
    const md: MediaDescriptor = {
      kind: 'video',
      fileId: 'vF',
      mime: 'video/mp4',
      ...
      width: 640,
      height: 480,
      localPath: '/abs/inbox/123-vid.mp4',
    }
    const out = renderMediaDescriptor(md)
    ...
    // Eagerly-downloaded bytes — topic sessions (no download_attachment) Read this.
    expect(out).toContain('local_path="/abs/inbox/123-vid.mp4"')
  })

  test('builds audio tag with local_path (eager download)', () => {
    const md: MediaDescriptor = {
      kind: 'audio',
      fileId: 'aF',
      name: 'track.mp3',
      mime: 'audio/mpeg',
      durationSec: 200,
      localPath: '/abs/inbox/123-track.mp3',
    }
    const out = renderMediaDescriptor(md)
    expect(out).toContain('kind="audio"')
    expect(out).toContain('name="track.mp3"')
    expect(out).toContain('local_path="/abs/inbox/123-track.mp3"')
  })
  ...
  test('builds video_note tag with local_path', () => {
    const md: MediaDescriptor = {
      kind: 'video_note',
      fileId: 'vnF',
      size: 1024,
      durationSec: 12,
      localPath: '/abs/inbox/123-note.mp4',
    }
    expect(renderMediaDescriptor(md)).toBe(
      '<media kind="video_note" file_id="vnF" size="1024" duration_sec="12" local_path="/abs/inbox/123-note.mp4" />',
    )
  })

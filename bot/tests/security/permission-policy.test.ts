...
const VARIANT1: PermissionPolicy = {
  ...
  confirm: {
    ...
    tools: ['mcp__office2-gbrain-tasks__task_done'],
  },
  allow: {
...
describe('systemctl is verb-aware — read-only verbs and mentions do not confirm (live FP 2026-06-10)', () => {
  test('read-only systemctl verbs auto-allow', () => {
    // `systemctl cat channel-agent-c.service` raised a real confirm card while
    // diagnosing a service — reading a unit file mutates nothing.
    expect(classify('Bash', { command: 'systemctl cat channel-agent-c.service' }, VARIANT1).tier).toBe('allow')
    ...
    expect(classify('Bash', { command: 'systemctl is-active office2-worker' }, VARIANT1).tier).toBe('allow')
  ...
  test('mutating systemctl verbs confirm, local and remote', () => {
    expect(classify('Bash', { command: 'systemctl restart office2-brain-swarm-worker' }, VARIANT1).tier).toBe('confirm')
...
describe('Variant 1 — smooth autonomy', () => {
  ...
  test('confirm-listed MCP tool asks for confirmation', () => {
    expect(classify('mcp__office2-gbrain-tasks__task_done', {}, VARIANT1).tier).toBe('confirm')
  })
})
...
describe('ultra-autonomy: lifting sudo / rm -rf NEVER lifts catastrophic hard-deny (Codex High 2026-06-10)', () => {
  // The owner's ultra-autonomy policy lifts sudo + rm -rf to run silently.

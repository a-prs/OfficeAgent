...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'office2-install-hooks-'))
...
describe('applyPatch (pure)', () => {
  ...
  test('permission gate: registers a PreToolUse gate entry FIRST, only on PreToolUse', () => {
    ...
    expect(pre[0]!.marker).toBe('office2-permission-gate-hook')
  ...
  test('permission gate: re-run replaces the gate entry, no duplicates', () => {
    ...
    const gate = (s.hooks.PreToolUse ?? []).filter((e) => e.marker === 'office2-permission-gate-hook')
  ...
  test('without permissionGateHelperPath, no gate entry is added (default off)', () => {
    ...
    expect((out.hooks.PreToolUse ?? []).some((e) => e.marker === 'office2-permission-gate-hook')).toBe(false)
  })
})

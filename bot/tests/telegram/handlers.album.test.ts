...
function makeStatePaths(): StatePaths {
  const root = mkdtempSync(join(tmpdir(), 'office2-album-test-'))
  return {
    root,
    env: join(root, '.env'),
...
// Build a minimal multichat policy that allowlists the owner user id
// in both the private chat AND a group chat. mention_allowlist defaults
// to just the owner so unaddressed group fragments aggregate to
// `addressedAtPush=false` and the FIX-D B1 silent-drop branch kicks in.
const ADDR_OWNER = 164795011
...
describe('FIX-D B1 — group album aggregate addressing', () => {
  ...
  test('DM album + router wired → legacy notify to master, NO router dispatch', async () => {
    ...
    // DM album must land in the master (channel-agent-c) session via legacy

...
function makeStatePaths(): StatePaths {
  const root = mkdtempSync(join(tmpdir(), 'office2-handlers-test-'))
  return {
    root,
    env: join(root, '.env'),
  ...
  return {
    ...
    message: {
      ...
      ...(opts.messageThreadId !== undefined
        ? { message_thread_id: opts.messageThreadId, is_topic_message: true }
        : {}),
    },
...
describe('handleInboundText — /model reachable from inside a topic (2026-07-30)', () => {
  test('/model typed directly inside an allowed topic is intercepted (OOB), not forwarded', async () => {
    ...
    // Short-circuited inline — no legacy/router notify (router isn't even
    // wired in this test's deps, so a fall-through would have been a
    // silent drop, not a throw — the real assertion is the reply below).
    expect(serverSpy.calls.length).toBe(0)
    // Reply landed in the TOPIC itself (composite chatId), not the master.
...
describe('handleInboundText — InboundWatcher (PR-A3)', () => {
  ...
  test('plain text from NOT allowed sender + busy session → watcher does NOT fire', async () => {
    ...
    // even though the session is «busy», because the owner explicitly

...
//     Stop hook so it means "Agent-c read it", not "the bot saw it".
...
function makeStatePaths(): StatePaths {
  const root = mkdtempSync(join(tmpdir(), 'office2-addr-test-'))
  return {
    root,
    env: join(root, '.env'),
...
describe('handleInboundText — no receive-time reaction (fix/eyes-on-read)', () => {
  ...
  // bot-receive time — it moved to the Stop hook so it means "Agent-c read
...
// private DM lands in the master (channel-agent-c) session via legacy
// notify — NOT a per-chat tmux session. Only group/supergroup chats are
// dispatched to the router. This is the owner's explicit topology:
// "main Telegram → channel-agent-c, group chats → multichat".
// ─────────────────────────────────────────────────────────────────────
...
describe('hybrid routing — DM to master, groups to per-chat (router wired)', () => {
  ...
  // Regression (2026-06-18): a voice message routed to a topic session used
  // to lose its transcript — the rendered <media kind="voice" .../> descriptor
  // was built but never placed on the InboundMessage (only `media_paths`, which
  // holds photos, was forwarded). The descriptor MUST ride along in
  // `media_descriptors` so the per-topic entrypoint can surface the transcript.
  test('voice in a forum topic → descriptor forwarded in media_descriptors', async () => {
    const { router, calls } = makeRouterSpy()
    const notifyCalls: Array<{ method: string; params: unknown }> = []
    const { deps, statePaths } = makeDepsWithRouter(router, makePolicy(), notifyCalls)
    // No GROQ_API_KEY in deps.env → transcription returns status=missing_key,
    // but the voice descriptor is still produced. We assert on the descriptor
    // being forwarded, which is the routing fix under test (independent of
    // whether Groq is reachable).
    const ctx = {
      chat: { id: ALLOWED_GROUP_CHAT_ID, type: 'supergroup' as const },
      from: { id: OWNER_USER_ID, is_bot: false, first_name: 'x', username: 'a_prisekin' },
      me: { id: 8507713167, username: 'canarybot' },
      message: {
        message_id: 77,
        date: 1700000000,
        ...
        chat: { id: ALLOWED_GROUP_CHAT_ID, type: 'supergroup' as const },
        from: { id: OWNER_USER_ID, is_bot: false, first_name: 'x', username: 'a_prisekin' },
        // Reply-to-bot satisfies the public-group addressing gate (a voice note
        // carries no @mention) — representative of replying inside a topic.
        reply_to_message: {
          message_id: 70,
          date: 1700000000,
          text: 'bot speaking',
          from: { id: 8507713167, is_bot: true, username: 'canarybot' },
        },
        voice: { file_id: 'voice-abc', duration: 7, mime_type: 'audio/ogg', file_size: 12345 },
      },
    } as unknown as Context

    await handleInboundVoice(ctx, deps)

    expect(calls.length).toBe(1)
    // Topic composite id carries the _t<threadId> suffix.
    expect(calls[0]?.chat_id).toBe(`${ALLOWED_GROUP_CHAT_ID}_t152`)
    const descriptors = calls[0]?.media_descriptors ?? []
    expect(descriptors.length).toBe(1)
    expect(descriptors[0]).toContain('kind="voice"')
    expect(descriptors[0]).toContain('transcription_status=')
    expect(notifyCalls.length).toBe(0)
    rmSync(statePaths.root, { recursive: true, force: true })
  })
})

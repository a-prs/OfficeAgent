...
describe('parseOobCommand', () => {
  ...
  test('parses /status@botname strips suffix', () => {
    const r = parseOobCommand('/status@office2canarybot', 'office2canarybot')
    expect(r).not.toBeNull()
...
// ─────────────────────────────────────────────────────────────────────
// /model (2026-07-27 base, 2026-07-30 inline-keyboard + per-topic extension)
// ─────────────────────────────────────────────────────────────────────

describe('/model', () => {
  function makeFakeModelSwitch(): NonNullable<OobContext['modelSwitch']> & { calls: string[] } {
    const calls: string[] = []
    return {
      calls,
  ...
  test('no args + modelSwitch configured → inline keyboard, no pending switch', async () => {
    const modelSwitch = makeFakeModelSwitch()
    const parsed = parseOobCommand('/model')!
    const result = await handleOobCommand(parsed, makeCtx({ modelSwitch }))
    expect(result.pendingModelSwitch).toBeUndefined()
    expect(result.replyToTelegram!.replyMarkup).toEqual({
      inline_keyboard: [[
        { text: 'GLM 5.2 (Z.ai)', callback_data: 'model:glm' },
        { text: 'Claude', callback_data: 'model:claude' },
      ]],
    })
    expect(modelSwitch.calls).toEqual([])
  })

  test('no args + modelSwitch NOT configured → "not configured" text, no keyboard', async () => {
    const parsed = parseOobCommand('/model')!
    const result = await handleOobCommand(parsed, makeCtx())
    expect(result.replyToTelegram!.replyMarkup).toBeUndefined()
    expect(result.replyToTelegram!.text).toContain('не настроено')
  })

  test('/model glm defers the actual switch (pendingModelSwitch), never calls switchTo synchronously', async () => {
    const modelSwitch = makeFakeModelSwitch()
    const parsed = parseOobCommand('/model glm')!
    const result = await handleOobCommand(parsed, makeCtx({ modelSwitch }))
    expect(result.pendingModelSwitch).toBe('glm')
    expect(modelSwitch.calls).toEqual([])
  })

  test('/model bogus-target with modelSwitch configured still asks usage (unchanged from no-args path is a keyboard; explicit garbage falls through the same way)', async () => {
    const modelSwitch = makeFakeModelSwitch()
    const parsed = parseOobCommand('/model bogus')!
    const result = await handleOobCommand(parsed, makeCtx({ modelSwitch }))
    // Any non-glm/claude arg — including garbage — takes the same "show
    // the keyboard" branch as no-args at all, since there's nothing useful
    // to parse out of it.
    expect(result.replyToTelegram!.replyMarkup).toBeDefined()
  })

  describe('in a topic (2026-07-30: /model typed directly inside a forum topic)', () => {
    const topicChatId = '-1009999999999_t555'

    test('no args in a topic → same inline keyboard as master (executeOobResult addresses ctx.chatId)', async () => {
      const modelSwitch = makeFakeModelSwitch()
      const parsed = parseOobCommand('/model')!
      const result = await handleOobCommand(parsed, makeCtx({ chatId: topicChatId, modelSwitch }))
      expect(result.pendingModelSwitch).toBeUndefined()
      expect(result.replyToTelegram!.replyMarkup).toEqual({
        inline_keyboard: [[
          { text: 'GLM 5.2 (Z.ai)', callback_data: 'model:glm' },
    ...
    test('/model glm in a topic with a dead pane → friendly "not running" reply, no defer, switchTo never called', async () => {
      const modelSwitch = makeFakeModelSwitch()
      const parsed = parseOobCommand('/model glm')!
      // No real tmux session named multichat-<topicChatId> exists in this
      // test environment, so the liveness check genuinely fails (exercises
      // the real `tmux -L default has-session` path, not a mock).
      const result = await handleOobCommand(parsed, makeCtx({ chatId: topicChatId, modelSwitch }))
      expect(result.pendingModelSwitch).toBeUndefined()
      expect(modelSwitch.calls).toEqual([])
      expect(result.replyToTelegram!.text).toContain('не запущен')
    })

    test('/model glm in the master DM still defers via pendingModelSwitch (topic branch must not affect master)', async () => {
      const modelSwitch = makeFakeModelSwitch()
      const parsed = parseOobCommand('/model glm')!
      const result = await handleOobCommand(parsed, makeCtx({ modelSwitch }))
      expect(result.pendingModelSwitch).toBe('glm')

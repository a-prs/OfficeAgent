...
describe('buildSanitizedTmuxEnv', () => {
  ...
  test('regex catches GBRAIN_* / OPENAI_* / ANTHROPIC_* / TELEGRAM_* prefixes — but not GBRAIN_BEARER', () => {
    const parent = {
      PATH: '/usr/bin',
      GBRAIN_BEARER: 'x',   // allowlisted exception for MCP auth — must NOT be caught
      GBRAIN_FOO: 'x',
      OPENAI_ORG: 'x',
    ...
    // GBRAIN_BEARER is allowlisted and must reach the child env.
    expect(childEnv.GBRAIN_BEARER).toBe('x')
  })

  test('GBRAIN_BEARER passes through; other GBRAIN_* are still blocked', () => {
    const parent = {
      PATH: '/usr/bin',
      GBRAIN_BEARER: 'my-bearer-token',
      GBRAIN_API_KEY: 'should-be-blocked',
      GBRAIN_SECRET: 'also-blocked',
    }
    const { childEnv, forbiddenSeen } = buildSanitizedTmuxEnv(parent)
    expect(childEnv.GBRAIN_BEARER).toBe('my-bearer-token')
    expect(childEnv.GBRAIN_API_KEY).toBeUndefined()
    expect(childEnv.GBRAIN_SECRET).toBeUndefined()
    expect(forbiddenSeen).toContain('GBRAIN_API_KEY')
    expect(forbiddenSeen).toContain('GBRAIN_SECRET')
    expect(forbiddenSeen).not.toContain('GBRAIN_BEARER')
  })
  ...
  test('MED-B #21: catches GITHUB_TOKEN + arbitrary GITHUB_* prefix', () => {
    const parent = {
      PATH: '/usr/bin',
      GITHUB_TOKEN: 'ghp_x',
      GITHUB_REPOSITORY: 'a-prs/repo', // prefix sweep (acceptable risk)
    }
    const { childEnv, forbiddenSeen } = buildSanitizedTmuxEnv(parent)
...
describe('TmuxSessionPool end-to-end env sanitization', () => {
  ...
  test('non-allowlisted but non-forbidden key (HOSTNAME) is dropped', async () => {
    ...
    process.env.HOSTNAME = 'agent-c-test-host'
    ...
    expect(env).not.toMatch(/^HOSTNAME=agent-c-test-host$/m)
    // And not in -e flags either.
    const argv = readFileSync(fixture.argvLog, 'utf8')
    expect(argv).not.toContain('HOSTNAME=agent-c-test-host')
  })
...
describe('spawn-chat-shell.sh standalone leak closure (FIX-A B2)', () => {
  test('TELEGRAM_BOT_TOKEN leaked via parent env does NOT survive `env -i` re-export', () => {
    const result = spawnSync(
      ...
      ['/usr/bin/env'],
      {
        env: {
          ...
          GBRAIN_BEARER: 'gbrain-bearer-for-mcp', // allowlisted — wrapper MUST propagate
    ...
    // GBRAIN_BEARER is allowlisted — it MUST survive the env -i wipe.
    expect(childEnv).toMatch(/^GBRAIN_BEARER=gbrain-bearer-for-mcp$/m)

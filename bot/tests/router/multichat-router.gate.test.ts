...
// ──────────────────────────────────────────────────────────────────────
// topics_only — General lobby guard (forum supergroup)
// ──────────────────────────────────────────────────────────────────────

describe('dispatch() topics_only General guard', () => {
  let fx: Fixture
  const GROUP = '-1004481335289'
  beforeEach(() => {
    fx = setupFixture()
  })
  afterEach(() => {
  ...
  test('General message (no _t suffix): no spawn, no dirs, one nudge sent', async () => {
    const policy = makePolicy({
      chats: {
        [GROUP]: makeChatPolicy({
          mode: 'public',
          respond_to_all: true,
          topics_only: true,
        }),
      },
      allowlist_users: ['448238861'],
      allowlist_chats: [GROUP],
    })
    const router = makeRouter(fx, policy)

    await router.dispatch({
      text: 'привет в General',
      chat_id: GROUP, // base id == General topic
      user_id: '448238861',
      user: 'owner',
      message_id: '162',
    ...
    // No session, no state dirs.
    expect(fx.pool.spawned).toEqual([])
    expect(existsSync(join(fx.stateDir, 'chats', GROUP))).toBe(false)
    // Exactly one script-side nudge, threaded to the offending message.
    expect(fx.telegram.calls.length).toBe(1)
    expect(fx.telegram.calls[0]?.chatId).toBe(GROUP)
    expect(String(fx.telegram.calls[0]?.text)).toContain('Topic')
    expect(fx.telegram.calls[0]?.opts).toMatchObject({ reply_to_message_id: 162 })
  })

  test('nudge is throttled: a burst into General yields a single reply', async () => {
    const policy = makePolicy({
      chats: {
        [GROUP]: makeChatPolicy({
          mode: 'public',
  ...
  test('real topic (_t suffix) still proceeds — guard does not fire', async () => {
    const policy = makePolicy({
      chats: {
        [GROUP]: makeChatPolicy({
          mode: 'public',
    ...
    const topic = `${GROUP}_t152`
    await router.dispatch({
      text: 'работаем в топике',
      chat_id: topic,
      user_id: '448238861',
      user: 'owner',
      message_id: '170',
    ...
    // Spawned the per-topic session; no nudge.
    expect(fx.pool.spawned).toEqual([topic])
    expect(existsSync(join(fx.stateDir, 'chats', topic, 'inbox'))).toBe(true)

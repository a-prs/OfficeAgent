...
// `streamingEnabled`, computed from the owner's chat id. If the
// owner DM had `streaming: 'progress'`, every chat — including a
...
//
// Surface under test:
//   * start(chatId, …) for owner vs. public group
...
describe('StatusManager multichat policy isolation', () => {
  test('owner chat with streaming=progress sends bubble; public group with streaming=off does not', async () => {
    const policy = makePolicy({
      [OWNER]: makeChatPolicy({ streaming: 'progress' }),
      [PUBLIC_GROUP]: makeChatPolicy({ streaming: 'off', mode: 'public' }),
    })
    const { mgr, api } = makeManager({ policy })

    const wHandle = await mgr.start(OWNER, undefined)
    const pHandle = await mgr.start(PUBLIC_GROUP, undefined)

    // Owner: real message id, entry tracked, network send fired.
    expect(wHandle.messageId).toBe(100)
    expect(mgr.isActive(OWNER)).toBe(true)
    const wSends = api.calls.filter(
      (c) => c.kind === 'send' && c.chatId === OWNER,
    )
  ...
  test('owner streaming continues to work alongside denied public group', async () => {
    // Smoke test that the gate does not break the happy path: a
    // shared manager handling both chats keeps emitting owner
  ...
  test('activeChatIds() only includes chats that were actually allowed to open', async () => {
    ...
    expect(mgr.activeChatIds()).toEqual([OWNER])
  })
...
describe('StatusManager legacy null-policy mode', () => {
  test('null policy: start(any_chat) sends bubble (legacy behaviour preserved)', async () => {
    ...
    const handle = await mgr.start(OWNER, undefined)

...
def test_issue_hmac_secret_creates_new():
    conn = FakeConn(row={"agent": "agent-g", "hmac_secret_sha256": None, "revoked_at": None})
    result = asyncio.run(
        ihs.issue_hmac_secret("agent-g", rotate=False, conn_factory=_factory(conn))
    )
    ...
    assert sent_args[1] == "agent-g"  # agent
...
def test_issue_hmac_secret_allows_rotate():
    ...
    result = asyncio.run(
        ihs.issue_hmac_secret("agent-g", rotate=True, conn_factory=_factory(conn))
    )
    assert result.status == ihs.IssueResult.OK
...
def test_issue_hmac_secret_db_error_returns_db_error():
    conn = FakeConn(
        row={"agent": "agent-g", "hmac_secret_sha256": None, "revoked_at": None},
...
def test_issue_hmac_secret_never_logs_raw_secret(capsys, monkeypatch):
    ...
    rc = ihs.main(["--agent", "agent-g"], conn_factory=_factory(conn))

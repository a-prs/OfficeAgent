...
def test_load_gateway_auth_parses_bearer_and_hmac(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert out["agent-g"].mode == "hmac"
    assert out["agent-g"].value == "raw-hmac-secret"
...
def test_agent_gateways_parser_backward_compat(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv(
        "AGENT_GATEWAYS",
        json.dumps({"claude": "http://example/claude", "agent-c": "http://example/agent-c"}),
    )
    monkeypatch.delenv("AGENT_GATEWAY_AUTH", raising=False)
    worker = _reload_worker(monkeypatch)
    gws = worker._load_gateways()
    assert gws == {"claude": "http://example/claude", "agent-c": "http://example/agent-c"}
...
def _make_row(to_agent: str = "claude", task_id: str = "t-1", from_agent: str = "agent-a") -> dict[str, Any]:
    return {
        "id": 1,
        "task_id": task_id,
...
def test_worker_signs_with_hmac_when_configured(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client = _RecordingClient()
    gateways = {"agent-g": "http://gw/agent-g"}
    auth_map = worker._load_gateway_auth()
    row = _make_row("agent-g")
...
def test_worker_body_unchanged_between_sign_and_post(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("AGENT_GATEWAY_AUTH", json.dumps({"agent-g": "hmac:env:HMAC_SECRET"}))
    worker = _reload_worker(monkeypatch)

    client = _RecordingClient()
    gateways = {"agent-g": "http://gw/agent-g"}
    auth_map = worker._load_gateway_auth()
    row = _make_row("agent-g", task_id="t-integ", from_agent="agent-a")
    ...
    assert decoded["agentId"] == "agent-g"
...
def test_worker_mixed_bearer_and_hmac_in_batch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client = _RecordingClient()
    gateways = {"agent-g": "http://gw/agent-g", "claude": "http://gw/claude"}
    auth_map = worker._load_gateway_auth()

    async def _go() -> None:
        s1, _ = await worker._deliver_one(client, gateways, _make_row("agent-g", task_id="t-1"), auth_map)
...
def test_worker_hmac_outbound_disabled_returns_retry(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    status, err = asyncio.run(worker._deliver_one(client, gateways, _make_row("agent-g"), auth_map))
    assert status == "retry"

...
async def test_audit_uses_authenticated_agent_not_param(
    ...
    """C1: HMAC-authenticated agent-g passes ``agent="agent-a"``; the
    decision tool MUST still stamp audit_log.agent == "agent-g" and
    surface "agent-a" only as ``declared_author`` in frontmatter.
    """
    from services.memory_mcp.tools import _REQUEST_AUTH, _authenticate_request

    secret = "c1-agent-g-secret"
    monkeypatch.setenv("GBRAIN_HMAC_SECRETS_JSON", f'{{"agent-g": "{secret}"}}')
    ...
    pool = _FakePool(
        hmac_rows=[_hmac_row("agent-g", secret, write_scopes=["30-decisions"])]
    )
    ...
    # The authenticated agent MUST be agent-g.
    assert agent_ctx.agent == "agent-g"

    # Now simulate the C1 fix at the tool level: even when the caller
    # passes ``agent="agent-a"``, the audit identity stays authenticated.
    spoofed_agent_param = "agent-a"
    ...
    assert resolved_agent == "agent-g", "audit must be authenticated agent"
    assert declared_author == "agent-a", "spoofed agent surfaces only as declared_author"
...
async def test_hmac_rejected_when_kill_switch_disabled(
    ...
    sig = _sign(secret, body, ts)
    av = HmacAuthValue(signature=sig, timestamp=str(ts), body=body)
    pool = _FakePool(hmac_rows=[_hmac_row("agent-g", secret)])
...
def test_restrict_read_scopes_intersects_with_token() -> None:
    ...
    ctx = AgentContext(agent="agent-g", write_scopes=[], read_scopes=["30-decisions"])
    # Allowed.
...
def test_recall_rejects_star_for_non_wildcard_token() -> None:
    ...
    full = AgentContext(agent="agent-a", write_scopes=[], read_scopes=["*"])
...
def test_check_read_scope_wildcard_and_explicit() -> None:
    ...
    wild = AgentContext(agent="agent-a", write_scopes=[], read_scopes=["*"])
    assert check_read_scope(wild, "anything") is True
    only = AgentContext(agent="agent-g", write_scopes=[], read_scopes=["30-decisions"])
...
async def test_recall_restricts_to_read_scopes_via_bearer(
    ...
    restricted = AgentContext(
        agent="agent-g", write_scopes=[], read_scopes=["30-decisions"]
    )
...
async def test_memory_create_decision_note_real_handler_via_hmac(
    ...
    assert filesystem write + audit identity are both ``agent-g``.
    """
    ...
    class _DecisionPool:
        ...
        async def fetch(self, query, *args):
            ...
            if "hmac_secret_sha256" in query:
                return [_hmac_row("agent-g", secret, write_scopes=["30-decisions"])]
    ...
    try:
        # Even with the spoofed ``agent`` param, audit MUST be agent-g.
        result = await create_decision(
            title="Test C1 audit",
            body="The body",
            tags=["test"],
            agent="agent-a",  # spoof attempt
        )
    finally:

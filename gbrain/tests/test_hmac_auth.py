...
async def test_hmac_constant_time_no_agent_ordering_leak(
    ...
    monkeypatch.setenv(
        ...
        f'{{"agent-g": "{secret}", "other": "other-secret"}}',
    )
    body = b"abc"
    ts = int(time.time())
    sig = _sign(secret, body, ts)
    rows = [
        _hmac_row("agent-g", secret),
    ...
    assert ctx.agent == "agent-g"
    # Exact accounting:
    #   - agent-g: env_hash==db_hash compare (+1), signature compare (+1) -> 2
...
# identity (or agent-a fallback). See DEVIATIONS.md → PRE-REVIEW FIX.
# ---------------------------------------------------------------------------
...
async def test_memory_mcp_create_decision_note_via_hmac_authenticates_correctly(
    ...
    HMAC agent, and audit_log would stamp that agent (not agent-a fallback).
    ...
    ``agent`` matches the HMAC identity, not agent-a.
    """
    from services.memory_mcp.tools import _REQUEST_AUTH, _authenticate_request

    secret = "s3cret-agent-g-mem-e2e"
    monkeypatch.setenv("GBRAIN_HMAC_SECRETS_JSON", f'{{"agent-g": "{secret}"}}')
    ...
    assert agent_ctx.agent == "agent-g", "HMAC agent must surface end-to-end"
    assert len(audit_calls) == 1
    assert audit_calls[0]["agent"] == "agent-g", (
        "audit_log.agent must reflect HMAC-authenticated agent, not agent-a"
    )
...
async def test_recall_mcp_recall_via_hmac_authenticates_correctly(
    ...
    secret = "s3cret-agent-g-recall-e2e"
    monkeypatch.setenv("GBRAIN_HMAC_SECRETS_JSON", f'{{"agent-g": "{secret}"}}')
